...
Infering on 100 patches should take around 30 seconds on the NUC.
"""
...
def main(args):
    """
    roslaunch locobot_control main.launch use_base:=true use_vslam:=true use_camera:=true use_arm:=true
    python project.py --n_grasps=5 --n_samples=100 --patch_size=100
    """

    """
    Visual SLAM for Obstacle Avoidance Navigation
    ...
    # explore room to show obstacle avoidance
    robot.base.go_to_relative([2.0, 0.0, 0.0], use_map=True, smooth=False, close_loop=True)
    robot.base.go_to_relative([2.0, 2.0, 0.0], use_map=True, smooth=False, close_loop=True)
    robot.base.go_to_relative([2.0, 2.0, 2.0], use_map=True, smooth=False, close_loop=True)

    # go to actual object's location (need to script and test in real world), assumed it already moved around at the start
    robot.base.go_to_relative([1.0, 0.0, 0.0], use_map=True, smooth=False, close_loop=True)
